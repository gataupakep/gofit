    ...
    <div class="row">
      <div class="col-md-12">
        <div class="card border-0 rounded shadow">
          <div class="card-body">
            <h3>Deposit Kelas EXP hari ini</h3>
            <router-link
            ...
            >
            <router-link
              :to="{ name: 'depositClass.indexAll' }"
              class="btn btn-md btn-warning"
              >SHOW ALL DATA DEPOSIT KELAS</router-link
            >
            <table class="table table-striped table-bordered mt-4">
              ...
              <tbody>
                <tr v-for="(depositClass, id) in depositClass_exp" :key="id">
...
export default {
  setup() {
    //reactive state
    let depositClass_exp = ref([]);

    //mounted
    onMounted(() => {
      //get API from Laravel Backend
      axios
        .get("https://jesso.ppcdeveloper.com/api/depositClass_exp")
        .then((response) => {
          //assign state posts with response data

          depositClass_exp.value = response.data.data;
        })
        .catch((error) => {
      ...
      swalWithBootstrapButtons
        ...
        .then((result) => {
          if (result.isConfirmed) {
            axios
              .delete(`https://jesso.ppcdeveloper.com/api/depositClass_exp/${id}`)
              .then(() => {
                //splice departemen
